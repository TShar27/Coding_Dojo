...
class User:
    ...
    def validate_user(user_data):
        ...
        if len(user_data['first_name']) == 0:
            ...
            is_valid = False
        elif len(user_data['first_name']) < 3:
            flash("First name needs to be greater than 2 characters") 
            is_valid = False
        if len(user_data['last_name']) == 0:
            flash("Last name cannot be empty") 
            is_valid = False   
        elif len(user_data['last_name']) < 3:
            flash("Last name needs to be greater than 2 characters")
            is_valid = False
        if len(user_data['email']) == 0:
            flash("Email name cannot be empty") 
            is_valid = False
